...
bibliography: 2025-06-04-Final.bib  
...
toc:
  ...
  - name: Background
    ...
    - name: GEMM and GEMV
    - name: NDP and PIM
    - name: PIM/NDP and GPU/NPU Accelater Features
    - name: Explanation of Detailed Operations in LLM Model
    - name: Distributed Deep Learning
    - name: Intra-node and Inter-node Communication
    - name: Collective Communication
    - name: SmartNIC
  - name: LLM Serving optimized ​for computational characteristics
    subsections:
      - name: NeuPIMs
      - name: PIM is All you need
      - name: Limitation
  - name: Optimizing communication overhead with SmartNIC
    subsections:
      - name: SmartNIC for Ring-AllReduce
      - name: Zero-Sparse AllReduce and SmartNIC offloading
      - name: Limitation of SmartNIC
  - name: Conclusion
...
PIM performs computations by directly manipulating the sense amplifiers inside the DRAM chip. Compared to NDP, it provides higher bandwidth and thus enables faster computations, but being inside the chip, it has lower computational flexibility and primarily performs MAC (Multiplication-ACcumulation) operations.<d-cite key="HBMPIM, LPDDR5PIM, NDP"></d-cite>   

NDP is typically located in the DRAM controller. Although it has lower bandwidth than PIM, it supports flexible data formats and performs general-purpose computations, offering a wide range of application possibilities.<d-cite key="NDP"></d-cite>    
...
### PIM/NDP and GPU/NPU Accelater Features
...
> Transformer & GPT
  ...
  LLMs such as ChatGPT and Llama3 <d-cite key="Llama3"></d-cite> primarily follow the GPT architecture, which adopts only the decoding stack of the Transformer structure. A key characteristic of this architecture is that it generates one token per iteration during the token generation process.<d-cite key="PIMIsAllYouNeed"></d-cite>   

  

> Difference Between the Prefill and Decoding Phases in the Decoding Stack

  {% include figure.html path="assets/img/2025-06-04-Final/attacc.png"  class="col-10" %}
  

  The transformer structure in GPT primarily consists of a Decoding Stack, which is composed of Multi-Head Attention (MHA) blocks and Feed Forward blocks. Each of these blocks is made up of the following key layers<d-cite key="attacc"></d-cite>:  
  - MHA Block
...
### Distributed Deep Learning
  ...
  {% include figure.html path="assets/img/2025-06-04-Final/DDL.png"  class="col-10" %}
...
As deep learning models continue to grow in size, it has become increasingly difficult to train large-scale models using only the memory and compute resources of a single GPU. One of the key approaches to overcoming this limitation is **Distributed Deep Learning** (DDL). DDL distributes model parameters or data across multiple GPUs or nodes, enabling parallel training. It is generally categorized into **Data Parallelism** and **Model Parallelism**.<d-cite key="denneman2020multiGPU"></d-cite>
  ...
  {% include figure.html path="assets/img/2025-06-04-Final/parallelism.png"  class="col-10" %}
...
### Intra-node and Inter-node Communication
  ...
  {% include figure.html path="assets/img/2025-06-04-Final/nodecomm.png"  class="col-10" %}
...
In DDL, **Collective Communication** is essential for synchronizing model parameters and efficiently distributing or aggregating data. This refers to communication patterns that involve exchanging or combining data across multiple processes or GPUs. These patterns are typically categorized into several common types, as outlined below.<d-cite key="wikiCollectiveOp"></d-cite>   
  {% include figure.html path="assets/img/2025-06-04-Final/CC.png"  class="col-10" %}
...
A **Network Interface Card** (NIC) is a hardware device used to connect a system to a network and enable communication. Traditional NICs are limited in functionality—they typically handle simple network-related operations or forward incoming packets to the host CPU. In contrast, a SmartNIC is an enhanced Ethernet device equipped with onboard processing cores, allowing it to support more general-purpose tasks. Recent research leveraging SmartNICs often focuses on offloading certain tasks from the host CPU, thereby reducing its workload and enabling more efficient communication.   
  ...
  {% include figure.html path="assets/img/2025-06-04-Final/smartnic.png"  class="col-10" %}
...
## LLM Serving optimized ​for computational characteristics
...
- Prefill  
  ...
  The input $$X:[\text{N}_{\text{prompt}}, d_{\text{emb}}]$$ is structured as a matrix, where $$\text{N}_{\text{prompt}}$$ denotes the number of prompt tokens, and $$d_{\text{emb}}$$ is the dimensionality of the embedding vector representing each token.
  - MHA Block
    - QKV Generation (GEMM):  
     $$W^Q, W^K, W^V : [\text{N}_{\text{prompt}}, d_{\text{emb}}] \times [d_{\text{emb}}, d_{\text{emb}}] = [\text{N}_{\text{prompt}}, d_{\text{emb}}]$$
    - Attention :   
    ...
    - Concat: $$\text{Concat}([head_1:[N_{prompt}, \frac{d_{emb}}{H}], ..., head_h:[N_{prompt}, \frac{d_{emb}}{H}]])$$   
    $$=[\text{N}_{prompt}, d_{emb}]$$
  - FF Block
    - Feed Forward 1: $$Z = XW_1+ b_1  \text{ where, } XW_1:[\text{N}_{prompt}, d_{emb}]\times[d_{emb},4\times d_{emb}]$$  
      is computed using GEMM.
    - GeLU: Simple scalar-wise multiplication and activation.
    - Feed Forward 2: $$\text{Output} = ZW_2 + b_2  \text{ where, } XW_2:[\text{N}_{prompt}, 4\times d_{emb}]\times[4\times d_{emb},d_{emb}]$$  
    $$=[N_{\text{prompt}}, d_{\text{emb}}]$$  
      is also computed as GEMM.
...
During the Decoding phase, each request reuses previously computed Key and Value matrices, which are stored in memory as KV Cache and concatenated back during computation. Since the Query corresponds to **only one newly generated token**, it exists in the form of a vector. As a result, the attention operation involves multiple GEMV computations along with heavy KV Cache memory loads, making this phase predominantly memory-bound.
  ...
  - MHA Block: Emphasis on **GEMV** operations increases
    ...
    - Concat:  
      $$\text{Concat}([head_1:[1, \frac{d_{emb}}{H}], ..., head_h:[1, \frac{d_{emb}}{H}]])$$  
      $$=[1, d_{emb}]$$   
  - FF Block
    - Feed Forward 1:  
      $$Z = XW_1+ b_1\text{ where, }XW_1:[1, d_{emb}]\times[d_{emb},4\times d_{emb}]$$  
...
{% include figure.html path="assets/img/2025-06-04-Final/GEMM-GEMV.png"  class="col-10" %}
...
- NeuPIMs <d-cite key="neupims"></d-cite> proposes a redesigned architecture where the memory functionality and GEMV computation of PIM can be executed in parallel. This allows the main accelerator (e.g., GPU) to focus on compute-intensive GEMM operations in the prefill phase, while the memory-bound GEMV operations in the decoding phase are offloaded to PIM units and processed asynchronously.

- In PIM is All You Need <d-cite key="PIMIsAllYouNeed"></d-cite>, the authors observe that the Prefill phase contributes only a small portion of the end-to-end LLM workload. Leveraging this, they propose eliminating power-hungry GPUs/TPUs and introduce a highly power-efficient LLM PIM serving system based on PNM (a type of NDP) and multiple PIM units to handle the entire pipeline efficiently.
...
{% include figure.html path="assets/img/2025-06-04-Final/neupimsOverlap.png"  class="col-10" %}
{% include figure.html path="assets/img/2025-06-04-Final/NeuPIMs_processing.png"  class="col-10" %}
...
- Models: Uses GPT3(7B, 13B, 30B, 175B), a state-of-the-art LLM developed by OpenAI. 
- Datasets : ShareGPT<d-cite key="sharegpt"></d-cite> and Alpaca<d-cite key="alpaca"></d-cite>
  - **ShareGPT** uses real-world user logs in ChatGPT and **Alpaca** uses instruction dataset generated by OpenAI's text-davanci-003 engine.
...
{% include figure.html path="assets/img/2025-06-04-Final/PIM-PNM.png"  class="col-10" %}
...
{% include figure.html path="assets/img/2025-06-04-Final/Results.png"  class="col-10" %}  
  - Models : Llama2-70B   
  - Datasets : **input 512 Tokens (Fixed)** and outputs 128 ~ 3.5K Tokens   
  - Framework : vLLM
...
In recent models such as **LLaMA3** and **DeepSeek LLM**, the traditional **Multi-Head Attention (MHA)** mechanism has evolved into variants like **GQA (Group-Query Attention)** and **MQA (Multi-Query Attention)** [GQA paper](https://arxiv.org/pdf/2305.13245) <d-cite key="GQA"></d-cite>. In **MQA**, all attention heads share a **single KV Cache**, whereas in **GQA**, groups of heads share **a subset** of the KV Cache.

{% include figure.html path="assets/img/2025-06-04-Final/MHA_GQA_MQA.png"  class="col-10" %}


As a result, during **decoding**, the query from a single head originally a **vector with a single row** is transformed into a **matrix** with $$\frac{\text{Origin Heads}}{\text{Group Size}}$$
...
{% include figure.html path="assets/img/2025-06-04-Final/RING_basic.png"  class="col-10" %}
...
{% include figure.html path="assets/img/2025-06-04-Final/RING_upgrade.png"  class="col-10" %}
...
{% include figure.html path="assets/img/2025-06-04-Final/RING_arch.png"  class="col-10" %}
...
{% include figure.html path="assets/img/2025-06-04-Final/RING_small_eval.png"  class="col-10" %}
...
{% include figure.html path="assets/img/2025-06-04-Final/RING_large_eval.png"  class="col-10" %}
...
{% include figure.html path="assets/img/2025-06-04-Final/SPARSE_algo.png"  class="col-10" %}
...
{% include figure.html path="assets/img/2025-06-04-Final/SPARSE_aggregator.png"  class="col-10" %}
...
The experiments were conducted on two types of systems: one with GPU workers and another with CPU workers. For the SmartNIC, the authors used the **NVIDIA BlueField-2**. <d-cite key="bluefield2"></d-cite> A custom microbenchmark, similar in spirit to the OSU MPI Microbenchmark suite, was used to evaluate AllReduce performance. Unlike typical AllReduce benchmarks, this version allows for configurable array sparsity, enabling more nuanced experimentation. For dense AllReduce comparisons, HPC-X MPI, Intel MPI, and NCCL were used as baselines. Due to hardware limitations, the testbed was restricted to a single BlueField-2 SmartNIC; therefore, scalability experiments were conducted using a combination of CPU servers and SmartNICs.      
...
{% include figure.html path="assets/img/2025-06-04-Final/image.png"  class="col-10" %}
...
{% include figure.html path="assets/img/2025-06-04-Final/SPARSE_result.png"  class="col-10" %}
...
We first examined the architectural aspect, analyzing the computational differences between the **prefill** and **decoding** phases in large language model (LLM) serving. This analysis highlights how decoding workloads introduce significant **memory bottlenecks**, primarily due to KV cache operations and the memory-bound nature of GEMV computations. Traditional GPU and TPU-based accelerators are optimized for **compute-bound operations (GEMM)** due to their high FLOPS capabilities. However, they face significant **bandwidth bottlenecks** when performing **memory-bound operations (GEMV)**, particularly during the decoding phase of LLMs where frequent KV cache loads dominate.
